...
LeRobot~\cite{lerobot2024} framework. Each joint angle from the IK
solver (in radians) is normalised to $a_{\text{norm}} \in [0, 1]$
using the joint range limits (Table~\ref{tab:joints}) and converted
to a motor command $a_{\text{motor}}$:
\begin{equation}
  a_{\text{motor}} =
  \begin{cases}
