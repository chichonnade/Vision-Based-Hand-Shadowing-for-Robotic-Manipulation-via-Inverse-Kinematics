...
due to hand occlusion by surrounding objects, highlighting both the
...
is preserved. Potential mitigation strategies include temporal Kalman
filtering to interpolate through brief occlusion episodes,
multi-camera setups to provide alternative viewpoints, and learned depth
completion to handle partial depth failures. These represent key
...
partial self-occlusion from the egocentric camera.
...
\begin{itemize}
  \item \textbf{Occlusion-robust hand tracking}: Temporal tracking
        with Kalman filtering or transformer-based models,
